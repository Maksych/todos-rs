    ...
    let onsubmit = {
        ...
        move |e: SubmitEvent| {
            ...
            spawn_local(async move {
                let token = api.sign_in(&form.username, &form.password).await;

                let user = api.profile_with_token(&token.access).await;

                store.dispatch(Action::SignIn { token, user });
            })
        }
    };
    ...
    html!(
        <div class="form-sign w-100 m-auto">
            <form { onsubmit }>
                ...
                <Link<Route> classes="nav-link text-center" to={Route::SignUp}>{ "Sign up" }</Link<Route>>
            </form>
        </div>
    )
}
